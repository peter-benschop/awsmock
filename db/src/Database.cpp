...
namespace AwsMock::Database {
    ...
    void Database::CreateDatabase(const std::string &dataDir, const std::string &dbFile){
        ...
        session << "CREATE TABLE s3_notification (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, region VARCHAR(255), bucket VARCHAR(255), notification_id INT, function VARCHAR(128), "
        ...
        session << "CREATE TABLE sqs_attribute (id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, message_id VARCHAR(100), name VARCHAR(128), value VARCHAR(255), "
                   "created DATETIME DEFAULT CURRENT_TIMESTAMP, modified DATETIME DEFAULT CURRENT_TIMESTAMP)",
            Poco::Data::Keywords::now;
