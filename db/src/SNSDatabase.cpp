...
namespace AwsMock::Database {
    ...
    bool SNSDatabase::TopicExists(const std::string &topicArn) {
        ...
        _logger.trace() << "Topic exists: " << (count > 0 ? "true" : "false");
        return count > 0;
    }
    ...
    Entity::SNS::Topic SNSDatabase::CreateTopic(const Entity::SNS::Topic &topic) {
        ...
        _logger.trace() << "Topic created, oid: " << result->inserted_id().get_oid().value.to_string();
    ...
    void SNSDatabase::DeleteTopic(const Entity::SNS::Topic &topic) {
       ...
       _logger.debug() << "Topic deleted, count: " << result->deleted_count();
   }
