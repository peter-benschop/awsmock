...
namespace AwsMock::Database::Entity::Module {
  ...
  void Module::FromDocument(mongocxx::stdx::optional<bsoncxx::document::value> mResult) {
  ...
  std::ostream &operator<<(std::ostream &os, const Module &m) {
    os << "Service={id='" << m.oid << "', name='" << m.name << "', executable='" << m.executable << "', port=" << m.port << ", status='" << ModuleStatusToString(m.status)
       << "', created=" << Poco::DateTimeFormatter().format(m.created, Poco::DateTimeFormat::HTTP_FORMAT) << "', modified=" << Poco::DateTimeFormatter().format(m.modified, Poco::DateTimeFormat::HTTP_FORMAT) << "}";
    return os;
  }
