...
void Manager::Initialize() {
  InitializeDatabase();
  log_info << "Starting " << Core::YamlConfiguration::GetAppName() << " "
           << Core::YamlConfiguration::GetVersion() << " pid: " << getpid()
           << " loglevel: "
           << Core::YamlConfiguration::instance().GetValueString(
                  "awsmock.logging.level");
  log_info << "Configuration file: "
           << Core::YamlConfiguration::instance().GetFilename();
  log_info << "Dockerized: " << std::boolalpha
           << Core::YamlConfiguration::instance().GetValueBool(
                  "awsmock.dockerized");
}

void Manager::InitializeDatabase() {
  // Get database variables
  if (Core::YamlConfiguration &configuration =
          Core::YamlConfiguration::instance();
      configuration.GetValueBool("awsmock.mongodb.active")) {
    const std::string name =
        configuration.GetValueString("awsmock.mongodb.name");
    const std::string host =
        configuration.GetValueString("awsmock.mongodb.host");
    const std::string user =
        configuration.GetValueString("awsmock.mongodb.user");
    const std::string password =
        configuration.GetValueString("awsmock.mongodb.password");
    ...
    // MongoDB URL
    const std::string url = "mongodb://" + user + ":" + password + "@" + host +
                            ":" + std::to_string(_port) +
                            "/?maxPoolSize=" + std::to_string(poolSize);
    ...
    pool.configure(std::move(instance),
                   bsoncxx::stdx::make_unique<mongocxx::pool>(std::move(_uri)));
    log_info << "MongoDB database initialized, version: "
             << mongocxx::v_noabi::options::server_api::version_to_string(
                    api.get_version());
...
void Manager::StopModules() {
  ...
  Database::ModuleDatabase &moduleDatabase =
      Database::ModuleDatabase::instance();
  for (Database::Entity::Module::ModuleList modules =
    ...
    if (module.state == Database::Entity::Module::ModuleState::RUNNING) {
      ...
      moduleDatabase.SetState(module.name,
                              Database::Entity::Module::ModuleState::STOPPED);
...
void Manager::Run() {
  ...
  Database::ModuleDatabase &moduleDatabase =
      Database::ModuleDatabase::instance();
  for (const std::map<std::string, Database::Entity::Module::Module>
           existingModules = Database::ModuleDatabase::GetExisting();
       const auto &module : existingModules) {
    ...
    if (configuration.GetValueBool("awsmock.modules." + module.first +
                                   ".active")) {
      moduleDatabase.SetStatus(module.first,
