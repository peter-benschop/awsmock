...
namespace AwsMock::Service {
    ...
    DockerService::DockerService() {
        ...
        _isDocker = _configuration.getBool("awsmock.docker.active");
        _containerSocketPath = _isDocker ? _configuration.getString("awsmock.docker.socket") : _containerSocketPath = _configuration.getString("awsmock.podman.socket");
    ...
    bool DockerService::ImageExists(const std::string &name, const std::string &tag) const {
        ...
        if (_isDocker) {
            ...
            if (const auto [statusCode, body] = _domainSocket->SendJson(http::verb::get, "http://localhost/v5.0.0/libpod/images/" + name + "/exists"); statusCode == http::status::no_content) {
    ...
    std::string DockerService::BuildImage(const std::string &name, const std::string &tag, const std::string &dockerFile) const {
        boost::mutex::scoped_lock lock(_dockerServiceMutex);
    ...
    void DockerService::DeleteImage(const std::string &id) const {
        ...
        if (auto [statusCode, body] = _domainSocket->SendJson(http::verb::delete_, "http://localhost/images/" + id + "?force=true"); statusCode != http::status::ok) {
    ...
    bool DockerService::ContainerExists(const std::string &id) const {
        ...
        if (_isDocker) {
            ...
            auto [statusCode, body] = _domainSocket->SendJson(http::verb::get, "http://localhost/v5.0.0/libpod/containers/" + id + "/exists");
            if (statusCode == http::status::ok) {
    ...
    bool DockerService::ContainerExists(const std::string &name, const std::string &tag) {
        ...
        if (_isDocker) {
            ...
            Core::DomainSocketResult domainSocketResponse = _domainSocket->SendJson(http::verb::get, "http://localhost/v5.0.0/libpod/images/" + name + "/exists");
    ...
    Dto::Docker::CreateContainerResponse DockerService::CreateContainer(const std::string &imageName, const std::string &tag, int hostPort, int containerPort) {
        ...
        Core::DomainSocketResult domainSocketResponse = _domainSocket->SendJson(http::verb::post, "http://localhost/containers/create?name=" + containerName, jsonBody);
        if (domainSocketResponse.statusCode != http::status::created) {
            log_warning << "Create container failed, httpStatus: " << domainSocketResponse.statusCode << " body " << domainSocketResponse.body;
            return {};
        }
    ...
    bool DockerService::NetworkExists(const std::string &name) {
        ...
        if (_isDocker) {
            ...
            Core::DomainSocketResult domainSocketResponse = _domainSocket->SendJson(http::verb::get, "http://localhost/v5.0.0/libpod/network/" + name + "/exists");
            if (domainSocketResponse.statusCode == http::status::no_content) {
    ...
    std::string DockerService::GetNetworkName() {
        if (Core::Configuration::instance().getBool("awsmock.docker.active")) {
            return Core::Configuration::instance().getString("awsmock.docker.network.mode");
        } else {
            return Core::Configuration::instance().getString("awsmock.podman.network.mode");
        }
    }
