...
namespace AwsMock::Core {
    ...
    void YamlConfiguration::DefineStringProperty(const std::string &key, const std::string &envProperty, const std::string &defaultValue) {
    ...
    void YamlConfiguration::DefineIntProperty(const std::string &key, const std::string &envProperty, int defaultValue) {
        if (getenv(envProperty.c_str()) != nullptr) {
            _yamlConfig[key] = envProperty.c_str();
            AddToEnvList(key, getenv(envProperty.c_str()));
        } else if (!HasProperty(key)) {
    ...
    void YamlConfiguration::SetValue(const std::string &key, const std::string &value) const {
        if (!HasProperty(key)) {
    ...
    void YamlConfiguration::SetValue(const std::string &key, const int value) const {
        if (!HasProperty(key)) {
            throw CoreException("Property not found, key: " + key);
        }
        std::vector<std::string> paths = StringUtils::Split(key, '.');
    ...
    std::string YamlConfiguration::GetValueString(const std::string &key) const {
        if (!HasProperty(key)) {
            throw CoreException("Property not found, key: " + key);
        }
        std::vector<std::string> paths = StringUtils::Split(key, '.');
    ...
    int YamlConfiguration::GetValueInt(const std::string &key) const {
        if (!HasProperty(key)) {
            throw CoreException("Property not found, key: " + key);
        }
        std::vector<std::string> paths = StringUtils::Split(key, '.');
    ...
    long YamlConfiguration::GetValueLong(const std::string &key) const {
        if (!HasProperty(key)) {
            throw CoreException("Property not found, key: " + key);
        }
        std::vector<std::string> paths = StringUtils::Split(key, '.');
    ...
    bool YamlConfiguration::GetValueBool(const std::string &key) const {
        if (!HasProperty(key)) {
            throw CoreException("Property not found, key: " + key);
        }
        std::vector<std::string> paths = StringUtils::Split(key, '.');
    ...
    double YamlConfiguration::GetValueDouble(const std::string &key) const {
        if (!HasProperty(key)) {
            throw CoreException("Property not found, key: " + key);
        }
        std::vector<std::string> paths = StringUtils::Split(key, '.');
    ...
    std::string YamlConfiguration::GetAppName() {
        return PROJECT_NAME;
    }

    std::string YamlConfiguration::GetVersion() {
    ...
    void YamlConfiguration::AddToEnvList(const std::string &key, const std::string &value) {
        _envList[key] = value;
    }
