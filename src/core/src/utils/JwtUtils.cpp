...
namespace AwsMock::Core {
    ...
    std::string JwtUtils::CreateToken(const std::string &privateKey, const std::string &issuer, const std::map<std::string, std::string> &payload) {
        ...
        token.sign(jwt::algorithm::rs256("", privateKey, "", ""));

        return token.sign(jwt::algorithm::rs256("", privateKey, "", ""));
    }

    bool JwtUtils::VerifyToken(const std::string &publicKey, const std::string &jwt, const std::string &issuer) {
        try {
